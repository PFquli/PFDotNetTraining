...
import {
  ...
  getItemsInFolder,
  getItemById,
} from '../data/dataOperation';
import {
  generateKey,
  getCurrentDate,
} from '../utilities/utilities-function';
...
ready(async () => {
  ...
  const modalEle = document.getElementById('add-modal');
...
function attachOnclickFolder(id, tr) {
  tr.addEventListener('click', async function() {
    ...
    const fold = await getItemById(id);
    clickedRow = id;
    const item = new Item();
    ...
    //    });
    // }
    const items = await getItemsInFolder(id);
    await generateData(items);
  });
}
/**
...
function addItemEvent(btn) {
  btn.onclick = async function() {
    ...
    const id = null;
    const creator = await getUserName();
    if (!editMode) {
      // Add file or folder
      const temp = {
        id,
      ...
      // }
      const file = await getItemById(hoverRow);
      const item = new Item();
...
function attachRemoveItemEvent(row) {
  ...
  for (let i = 0; i < btn.length; i += 1) {
    btn[i].addEventListener('click', async function() {
      ...
      //    folder.remove();
      // }
      const fold = await getItemById(hoverRow);
      const item = new Item();
      item.mapping(fold.data);
...
function checkboxEvent() {
  ...
  inputElem.onclick = function() {
    const isFile = inputElem.checked;
    const prefix = isFile
      ? properties.FILE_PREFIX
      : properties.FOLDER_PREFIX;
    const result = generateKey(prefix, randomLength);
...
function attachGoUpEvent() {
  document.getElementById('go-up-button').onclick = async function() {
    if (clickedRow !== properties.BASE_ID) {
      ...
      //    });
      // }
      const parent = await getItemById(clickedRow);
      const item = new Item();
